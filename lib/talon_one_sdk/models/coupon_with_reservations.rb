...
module TalonOne
  class CouponWithReservations < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::CouponWithReservations` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::CouponWithReservations`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@value.nil? && @value.to_s.length < 4
        invalid_properties.push('invalid value for "value", the character length must be greater than or equal to 4.')
      end
    ...
    def value=(value)
      ...
      if value.to_s.length < 4
        fail ArgumentError, 'invalid value for "value", the character length must be greater than or equal to 4.'
      end
    ...
    def hash
      [id, created, campaign_id, value, usage_limit, discount_limit, reservation_limit, start_date, expiry_date, limits, usage_counter, discount_counter, discount_remainder, reservation_counter, attributes, referral_id, recipient_integration_id, import_id, reservation, batch_id, is_reservation_mandatory, implicitly_reserved, reservations].hash
    end
