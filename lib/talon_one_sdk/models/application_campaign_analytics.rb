...
module TalonOne
  class ApplicationCampaignAnalytics < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::ApplicationCampaignAnalytics`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @campaign_tags.length > 50
        invalid_properties.push('invalid value for "campaign_tags", number of items must be less than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if @campaign_tags.length > 50
    ...
    # Custom attribute writer method with validation
    # @param [Object] campaign_tags Value to be assigned
    def campaign_tags=(campaign_tags)
      if campaign_tags.nil?
        fail ArgumentError, 'campaign_tags cannot be nil'
      end

      if campaign_tags.length > 50
        fail ArgumentError, 'invalid value for "campaign_tags", number of items must be less than or equal to 50.'
      end
