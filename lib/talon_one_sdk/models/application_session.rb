...
module TalonOne
  class ApplicationSession < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::ApplicationSession`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def profileintegrationid=(profileintegrationid)
      if profileintegrationid.nil?
        fail ArgumentError, 'profileintegrationid cannot be nil'
      end

      if profileintegrationid.to_s.length > 1000
    ...
    # Custom attribute writer method with validation
    # @param [Object] cart_items Value to be assigned
    def cart_items=(cart_items)
      if cart_items.nil?
        fail ArgumentError, 'cart_items cannot be nil'
      end
