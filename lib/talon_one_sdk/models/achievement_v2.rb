...
module TalonOne
  class AchievementV2 < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::AchievementV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def subscribed_applications=(subscribed_applications)
      if subscribed_applications.nil?
        fail ArgumentError, 'subscribed_applications cannot be nil'
      end

      if subscribed_applications.length < 0
        fail ArgumentError, 'invalid value for "subscribed_applications", number of items must be greater than or equal to 0.'
      end
    ...
    def hash
      [id, created, name, title, description, target, period, recurrence_policy, activation_policy, fixed_start_date, end_date, allow_rollback_after_completion, subscribed_applications, user_id, created_by, period_end_override, has_progress, status, sandbox, timezone].hash
    end
