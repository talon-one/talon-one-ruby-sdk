...
module TalonOne
  class CatalogsStrikethroughNotificationPolicy < ApiModelBase
    # Notification name.
    attr_accessor :name
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::CatalogsStrikethroughNotificationPolicy`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def ahead_of_days_trigger=(ahead_of_days_trigger)
      if ahead_of_days_trigger.nil?
        fail ArgumentError, 'ahead_of_days_trigger cannot be nil'
      end

      if ahead_of_days_trigger > 30
        fail ArgumentError, 'invalid value for "ahead_of_days_trigger", must be smaller than or equal to 30.'
      end

      if ahead_of_days_trigger < 1
