...
module TalonOne
  class CustomerReservation < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::CustomerReservation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::CustomerReservation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reserved_at')
        self.reserved_at = attributes[:'reserved_at']
      end
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] closed_sessions Value to be assigned
    def closed_sessions=(closed_sessions)
      if closed_sessions.nil?
        fail ArgumentError, 'closed_sessions cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] total_sales Value to be assigned
    def total_sales=(total_sales)
      if total_sales.nil?
        fail ArgumentError, 'total_sales cannot be nil'
      end
    ...
    def hash
      [id, created, integration_id, attributes, account_id, closed_sessions, total_sales, loyalty_memberships, audience_memberships, last_activity, sandbox, reserved_at].hash
    end
