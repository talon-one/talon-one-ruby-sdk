...
module TalonOne
  ...
  class SupportRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::SupportRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      request_status_validator = EnumAttributeValidator.new('String', ["pending_approval", "approved", "rejected"])
    ...
    def request_status=(request_status)
      validator = EnumAttributeValidator.new('String', ["pending_approval", "approved", "rejected"])
    ...
    def hash
      [id, application_id, campaign_id, loyalty_program_id, subledger_id, created_by_user, created_at, customer_profile_id, request_type, request_value, request_note, request_status, processed_at, processing_note, processed_by_user].hash
    end
